...
export class JwtService {
  ...
  login(userData: User) {
    ...
    queryParams.append('email', em);
    queryParams.append('password', pw);
  ...
  verifyEmail(email: string) {
    let response = 'false';
    return this.httpClient.put<any>(`${this.apiUrl}/auth/forgotPassword`, {
      email: email,
    });
  }
  changePassword(userData: Password) {
    ...
    return this.httpClient.put<any>(`${this.apiUrl}/auth/updatePassword`, {
      email: em,
      password: pw,
      confirmPassword: cpw,
    });
